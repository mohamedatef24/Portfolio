    ...
    <div className="w-full max-w-md sm:max-w-lg md:max-w-2xl lg:max-w-4xl mx-auto">
      ...
      >
        <button
          type="button"
          onClick={goPrev}
          className="shrink-0 w-9 h-9 rounded-full bg-surface/90 text-accent border border-[var(--border)] flex items-center justify-center cursor-pointer hover:bg-accent hover:text-on-accent transition-colors"
        ...
        <div className="relative flex-1 min-w-0 h-[min(75vh,560px)] overflow-hidden rounded-2xl bg-card transition-shadow duration-300 border border-[var(--border)] shadow-[var(--shadow-glow)] hover:shadow-[var(--shadow-glow-hover)]">
          <Image
            key={index}
            ...
            style={{
              ...
              transform: animating ? `translateX(${exitOffset})` : "translateX(0)",
              transition: `transform ${ANIMATION_MS}ms cubic-bezier(.4,2,.6,1), opacity ${ANIMATION_MS}ms`,
            }}
          />
          <Image
            ...
            style={{
              ...
              transform: animating ? "translateX(0)" : `translateX(${enterOffset})`,
              transition: `transform ${ANIMATION_MS}ms cubic-bezier(.4,2,.6,1), opacity ${ANIMATION_MS}ms`,
            }}
          />
        </div>
